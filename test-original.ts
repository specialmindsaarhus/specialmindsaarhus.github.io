...
interface DecoratedCookie extends Cookie {
  icing: string;
  sprinkles: boolean;
}
...
export class AlgorithmExecutor {
  ...
  private processCookieWithAlgorithm(cookie: DecoratedCookie, lines: string[]): void {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      const trimmedLine = line.toLowerCase().replace(/[:"]/g, '');
      ...
      while (conditionStack.length > 0 && currentIndent <= conditionStack[conditionStack.length - 1].indent) {
        conditionStack.pop();
      }
